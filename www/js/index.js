    ...
    for (i = 0; i < matchPositions.length; i++) {
      ...
      while (matches.length > 0) {
        players[matches.pop()].element.attr(
          'transform',
          buildTranslate(position, startOffset + index * offset, startOffset + index * offset)
        );
        index++;
      }
    }
    ...
    for (var y = 0; y < config.tilesPerAxis; y++) {
      for (var x = 0; x < config.tilesPerAxis; x++) {
        ...
        gameBoard
          ...
          .attr(
            'y',
            gameBoardSize - y * tileSize + (onTop ? -tileSize + 3 * textOffset : -textOffset)
          )
